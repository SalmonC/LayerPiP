...
覆盖了本轮两个问题（① 热力条只在控制栏激活时显示；② 小窗内「更多播放设置」的面板落点）。
因此 0.2.19 是一个**用户确认可用**的版本，已打标签 `verified/0.2.19`。
注意：问题②在自动化里没能复现，用户侧现已正常；若日后再次出现，需收集：
**扩展版本 + 具体点击路径**（小窗控制栏齿轮→更多播放设置？网页浮动入口齿轮？字幕菜单→字幕设置与视频关联？）
+ 当时是增强小窗还是原生小窗。
