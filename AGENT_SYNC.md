...
- 版本（package.json / dist）：`0.2.18` / `0.2.18`
- dist SHA-256：`99147f028cacfc193a6230fa03310807135a329e0655b54401f1f8ec4bd18865`（构建脚本自报；未独立回读）
- 最近交付提交：`d0d16b2`（feat(highEnergyBar)：已看区间优先同步 B 站官方记录）
...
**无。** 特性 1（高能进度条 + 已看双色，含同步 B 站官方已看记录）已实现、已构建交付 **0.2.18**、已提交，工作区干净。
...
**测试时的一个坑（供接手者省时间）**：`.start-pip-btn` 是由 **mousemove** 惰性创建的。
自动化测试里必须先 `page.mouse.move()` 划过播放器区域，否则永远查不到该按钮——
本轮在这个坑上花了很久。另外构建脚本会拒绝「同版本重发不同产物」
（`Changed artifact needs a new package.json version`），改了源码就要递增版本号。
