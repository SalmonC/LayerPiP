...
- 0.2.22 源码已提交：`8d366a5`；用户教程/截图：`d1c23be`；上传验证记录：`df49ab3`。运行产物未改。
- 工作区：本轮已将上一轮所有待提交改动与新增使用文档分组提交，收工时干净。
- GitHub：`https://github.com/SalmonC/LayerPiP`，独立 **PRIVATE** 仓库，默认分支 `main`。本地仍是 `feature/layerpip-dual-mode`，通过 remote `layerpip` 推送到 main。原 `origin` 保持上游地址，不向其推送。
- Git：用户本轮明确授权上传；分组提交并 push，交接文件仍单独提交。浅克隆已 fetch --unshallow 补齐，未 rebase/amend/reset/force push，未推送标签或创建 Release。
...
**本轮私有仓库上传与使用文档已完成，无上传中断；整体多画面特性仍未全部做完。** 源码已提交并上传，旧记录的“源码未提交”已过期。面向使用者的入口是 `readme.md` → `docs/USER_GUIDE.md`；后续开发继续读 `docs/code-review-and-multi-video-2026-09-20.md`。
...
5. 仓库保持私有；用户要求正式版准备好后再开放，本轮不开放。教程没有虚构正式版下载，内测安装包仍由维护者提供。
...
### 2026-09-20 · Codex · 上传私有 GitHub 仓库并编写使用教程

- 做了什么：创建 SalmonC/LayerPiP 私有仓库；提交此前待提交的 0.2.22 源码；重写使用者首页，新增使用教程与三张真实 Edge 设置截图，保留上游署名/许可证与致谢。截图后恢复原播放界面，未修改播放参数。
- 提交：`8d366a5` 源码；`d1c23be` 文档/截图；`df49ab3` 上传验证记录。本交接文件单独提交，hash 用 git log -1 -- AGENT_SYNC.md 查询。
- git 操作：新增 layerpip remote，HEAD 推送到其 main；首次因浅克隆缺父提交失败，补齐上游历史后成功。无历史改写、宽泛暂存、tag、Release；原 origin 保持不变。
- 交付：GitHub PRIVATE、main 默认分支；dist 沿用 0.2.22，SHA-256 `c02c60665882de5bdec4c97fc1fbb2eb809e1eeb82a4c7af990d4f079f38d5ff`，未构建/重载。所有本地产物与回退资料均未上传也未删除。
- 验证：文档差异和本地链接/图片检查；3706 个历史 blob 未发现高置信凭据特征；远端源码、教程及截图 blob 与本地一致，远端树无 dist/.delivery/test-results。文档任务不重跑单元测试。
- 遗留：功能剩余范围见第 2 节；真实站点验收、正式版发布和开放仓库均未执行。截图来自当时已运行的小窗，不代表最新 0.2.22 的运行验收。
