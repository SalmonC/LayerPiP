...
## 2026-09-20 私有 GitHub 仓库与使用文档

- 用户明确授权上传 GitHub、保持私有并允许界面截图。创建独立仓库 `SalmonC/LayerPiP`，已核对 PRIVATE；原 origin 仍指向上游，使用新增 layerpip remote 上传到 main，不改写历史。
- 首次推送因浅克隆缺失父提交被 GitHub 拒绝；执行 `git fetch --unshallow --no-tags origin` 补齐上游历史后，普通 push 成功，未重写历史。补齐后扫描 3706 个历史 blob，未发现高置信凭据特征；远端源码、教程、三张图片的 Git blob 与本地一致，受保护产物不在远端树中。
- 将上一轮 0.2.22 的已验证源码单独提交为 `8d366a5`，此前“源码未提交”的状态到此结束。
- 重写 readme.md 为面向使用者的首页，新增 docs/USER_GUIDE.md，保留原作者署名/许可与引用致谢。明确内测包与源码 ZIP 的区别，不虚构正式版下载。
- 使用已打开的 Edge 小窗实拍播放、字幕、弹幕设置三图；未修改参数、未切换视频、未启用 AI，拍摄后关闭设置。运行中的页面不等于最新 dist，教程注明内测界面可能有差异，不将截图作为 0.2.22 实机验收证据。
- 验证限于文档差异、链接/图片存在性、历史凭据特征扫描（744 个 blob，未发现匹配）和远端上传回读。没有重复执行单元测试或重新构建。
- dist 仍为 0.2.22，SHA-256 `c02c60665882de5bdec4c97fc1fbb2eb809e1eeb82a4c7af990d4f079f38d5ff`。dist、所有回退副本、.delivery、test-results 未暂存也未删除；没有创建 Release 或推送标签。

## 2026-09-20 审阅 0.2.20，交付 0.2.22 双画面首版

- 修复高能曲线开关响应、重开监听、换源和异步串 CID、跨标签页观看记录并发覆盖、非法曲线响应误判/超大分配、菜单相对高度与 IndexedDB 迟到连接；补齐四语文案。
- 新增双路入口：B 站增强小窗 → 播放设置 → 多画面。原播放器 + 一路受限视频页，独奏、当前控制画面标记、全局命令路由、失败释放与退出还原。四路、拖动分隔、记忆和多路 AI 尚未实现。
- rc-util 补丁原本已接入；新增 webext-bridge 的 BFCache 断连 lastError 补丁，离线安装并核对锁文件。未实际触发用户 Edge BFCache，不声称错误列表已清空。
- 验证：源码定向 lint 通过；类型诊断与 0.2.20 一致（92 项旧错误，比较时归一化依赖补丁路径）；键盘回归通过。两个隔离夹具覆盖采集生命周期与多画面 DOM/音频控制，未操作用户浏览器或生产数据。日志/夹具保存在 .delivery/review-*。
- 0.2.21 曾完成构建与回读；随后补充“控制中”提示，按版本规则递增为 **0.2.22**。固定 `dist` SHA-256：`c02c60665882de5bdec4c97fc1fbb2eb809e1eeb82a4c7af990d4f079f38d5ff`，独立回读一致；扩展 ID 不变。
- 此次回退：`.delivery/rollback-0.2.21-2026-09-19T17-37-49-041Z` 与 `.delivery/safety-0.2.21-2026-09-19T17-37-49-041Z`，独立回读均为 `8fad6c89136a7e18c283510ca5e3dcea0fd50e5d7530d16a01af576996ea8bbd`（0.2.21）。0.2.20 双备份也保留并已独立回读，详见上一份 0.2.21 receipt。
- Git：源码基于 `ffc314d`，本轮源码与说明保留为明确的未提交改动；仅按明确授权单独提交 AGENT_SYNC.md。未 push、未打标签、未改写历史。
- 构建交付不代表已重载或用户验收；当前实现和后续边界见 docs/code-review-and-multi-video-2026-09-20.md。

## 2026-09-18 弹层随内容重排 + 去掉重复的「历史字幕」，交付 0.2.20

用户报三个问题。提交 `4a2f80f`；`dist` 已构建并替换为 **0.2.20**（SHA-256 `bfd6f3ab3d9df7967a9962119dbaeac2c24e9307ef5ead5174aaf29ab1747d3d`）。

### 问题 1：弹幕菜单展开「高级设置」后新增内容看不见、收起时位置不回收

**根因**：`rc-trigger` **不会因为弹层内容尺寸变化而重新对齐**。展开后弹层变高，位置却停在原处，新增内容落到可视区之外；收起时也不会回收。另外可用高度被写死为 `player.clientHeight - 60`，在小窗里白白浪费一截空间。

**修复**（`src/components/Dropdown/index.tsx`、`DanmakuSettingBtn.tsx`）：

- 对**弹层自身**加 `ResizeObserver`，尺寸变化时调用 `trigger.forceAlign()` 重排，并重算可用高度；
  为避免"设 max-height → 弹层尺寸变 → 再次触发"的自激循环，加"值没变就不重设"的短路。
- 可用高度改为按**触发按钮上边缘到播放器顶部的真实距离**计算（原来是 `clientHeight - 60`）。
- 弹幕「高级设置」展开后调用 `scrollIntoView({ block: 'nearest' })`，把新内容滚入可视区。

**实测（480×320 小窗）**：

| | 展开前 | 展开后 | 收起后 |
| --- | --- | --- | --- |
| 弹层 top | 47 | **4**（已重排） | **47**（位置回收） |
| 菜单滚动 | 0 | **7px**（新内容滚入） | 0 |
| 可用高度 | 260 → **272px** | | |
| 新增输入框在可视区内 | — | **true** | — |

### 问题 2：齿轮选单里的「历史字幕」与控制栏按钮重复

齿轮选单（`.fc-quick-settings`）里有一个「历史字幕」开关，与控制栏的 `SubtitleHistoryButton` 是**同一个配置项**（`subtitle_historyEnabled`），出现两个入口。已删除选单里的那个，保留控制栏按钮作为唯一入口。

实测齿轮选单现在为：`滚轮调节音量 / 适配窗口 R / 保存视频截图 / 快捷键 / 更多播放设置`。
顺带修了同一文件里既有的 `import/order` lint 报错。

### 问题 3：齿轮选单 → 快捷键 点进去什么都没有 —— 未能复现

两条路径都实测有内容：

| 路径 | 结果 |
| --- | --- |
| 齿轮选单 → 快捷键（弹窗） | 正常渲染，**21 行 / 298 字**（Space 播放/暂停 … 0 倍速模式/重置倍速） |
| 设置面板 → 快捷键标签页 | **9 条**快捷键 + 说明 + 「恢复安全默认」 |

因此**未改任何代码**（找不到可改的缺陷）。怀疑与上一轮一样是**旧注入脚本**所致——改完必须重新加载扩展**并刷新 B 站页面**。已列入待用户复测，并记录需要收集的信息（哪条路径、弹窗是否出现在网页、扩展版本）。

### 验证

`tsc` 全仓 92 条 == 基线；改动文件 eslint 通过；0 控制台错误。
...
提交 `d0d16b2`；固定 `dist` 已构建并替换为 **0.2.18**（SHA-256 `99147f028cacfc193a6230fa03310807135a329e0655b54401f1f8ec4bd18865`）。**未安装、未重新加载。**

### 实现

- 新增 `src/core/HighEnergyBar/officialWatched.ts`：读取 B 站播放器写在**页面源** IndexedDB 的已看数据——
  库 `pbp3`、对象存储 `pbpZebraCache`、`keyPath: "cid"`、记录 `{cid, data:[[startRatio,endRatio]], expire}`，
  `data` 是**相对时长的比例**，需 ×`duration` 换算成秒；到期 30 天；`expire` / `expireTime` 两种字段名都读。
- **只读、不写回、不留下副作用空库**：先用 `indexedDB.databases()` 探测存在性，再只读打开，
  并在 `onupgradeneeded` 里中止升级兜底（直接 `open()` 在库缺失时会顺手创建空库）。
- 控制器把数据分成两份：`officialRanges`（只读、只驻内存、不并入我们的存储）与 `ownRanges`（照旧按 cid 持久化），
  渲染用两者并集 —— **官方优先保证与 B 站一致**，自己的记录作为补充（例如只在小窗里看过、或 B 站热力条关闭时的观看）。
- 官方数据需要 `duration` 才能换算，切 cid 时可能还没拿到，因此在 `durationchange` 补读一次。

### 验证（真实浏览器，两阶段隔离）

| 阶段 | 结果 |
| --- | --- |
| A：不开小窗只看视频 | B 站自己写出 `pbp3 = [[0, 0.1277], [0.6, 0.6821]]`；同时**扩展自己的已看键为空**（隔离成立） |
| B：新标签页（全新 video 元素）打开小窗 | 本会话 `played` 仅 `[[0.2, 15.5]]`，但已看路径为 `M 0.00→127.70` 与 `M 600.00→682.10`，与官方比例 ×1000 **逐位吻合**，中间空档 127.7→600 保持灰色；0 控制台错误 |

`tsc` 全仓 92 条 == 基线；改动文件 eslint 通过。

### 过程中的两个坑（已记入 `AGENT_SYNC.md`）

1. `.start-pip-btn` 由 **mousemove** 惰性创建，自动化测试必须先 `page.mouse.move()` 划过播放器，否则永远查不到。
2. 构建脚本会拒绝「同版本重发不同产物」（`Changed artifact needs a new package.json version`）。本轮先构建 0.2.17 完成验证，之后仅一处 prettier 格式化改动，故按规则递增到 0.2.18（两次构建行为等价）。

### 遗留

用户实机验收；`.fc-heatbar` 的 `bottom/height` 视觉核对；`ja/ko/fr/es` 文案待译。
